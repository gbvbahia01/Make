...
public class MakeStringTest extends TestCase {
  ...
  public void testGetString_int_int() {
    ...
    for (int min = 1; min < 50; min++) {
      for (int max = min + 1; max < 100; max++) {
        ...
        assertTrue("String erro: Max: " + max + " Min:" + min + " Result: " + result.length(),
            (result.length() >= min) && (result.length() <= max));
        logger.debug("String - Max: " + max + " Min:" + min + " length: " + result.length()
            + " Result:" + result);
      }
    }
  }
  ...
  public void testGerarSenha_int_int() {
    ...
    for (int min = 1; min < 50; min++) {
      for (int max = min + 1; max <= MakePassword.MAX_LENGTH_SUPPORTS; max++) {
        ...
        assertTrue("Senha erro: Max: " + max + " Min:" + min + " Result: " + result.length(),
            (result.length() >= min) && (result.length() <= max));
        logger.debug("Senha - Max: " + max + " Min:" + min + " length: " + result.length()
            + " Result:" + result);
      }
    }
  }
  ...
  public void testGetString_int_all() {
    ...
    for (int i = 1; i < 100; i++) {
      ...
      assertTrue("String erro: Resultado" + result.length() + " Esperado: " + i,
          result.length() <= i);
      logger.debug("String - Max: " + i + " Lenght: " + result.length() + " Result: " + result);
    }
  }
  ...
  public void testGetString_int_letter() {
    ...
    logger.debug("String - Max: " + -50 + " Lenght: " + result.length() + " Result: " + result);
    result = MakeString.getLoren(5000);
    logger.debug("String - Max: " + 5000 + " Lenght: " + result.length() + " Result: " + result);
  ...
  public void testGetString_int_number() {
    ...
    for (int i = 1; i < 100; i++) {
      ...
      for (char c : result.toCharArray()) {
        assertTrue("String contém um caracter numérico inválido", Character.isDigit(c));
      }
    }
  }
  ...
  public void testGerarSenha_int() {
    ...
    for (int i = 1; i < MakePassword.MAX_LENGTH_SUPPORTS; i++) {
      ...
      assertTrue(result.length() <= i);
      assertTrue("Senha erro: Resultado" + result.length() + " Esperado: " + i,
          result.length() <= i);
      logger.debug("Senha - Max: " + i + " length: " + result.length() + " Result:" + result);
    }
  }
  ...
  public void testGerarEmail() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      logger.debug("Email - Inicio: " + inicio + " Fim: " + fim + " length: " + email.length()
          + " Result:" + email);
      assertNotNull("Email Inicio nulo", inicio);
      assertNotNull("Email Fim nulo", fim);
      assertTrue("Email inválido: " + email, EmailValidator.getInstance().isValid(email));
    }
  }
}
